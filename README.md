...
4. Users are now ready for combination. They can view the A/V segments and generate / iterate on variants via a *preview* while modifying user controls, adding desired variants to the render queue.
    ...
    * User Controls for video variant generation:
        ...
        * Users can then click `Generate` to generate variants accordingly, which will query language models on Vertex AI (Gemini Pro) with a detailed script of the video to generate potential variants that fulfill the optional user-provided prompt and target duration.
