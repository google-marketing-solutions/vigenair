...
* Transcription can either be done via Gemini or the [faster-whisper](https://github.com/SYSTRAN/faster-whisper) library, which uses OpenAI's Whisper model under the hood. This is controlled by modifying the `frontend` configuration property `CONFIG.defaultTranscriptionService` in [config.ts](ui/src/config.ts) to 'gemini' or 'whisper', respectively, then redeploying the UI. Vigenair defaults to using Whisper for its optimal transcription quality; the [small](https://github.com/openai/whisper#available-models-and-languages) multilingual model provides the best quality-performance balance. However, if it doesn't work well for your target language, you can change the model by updating the `CONFIG_TRANSCRIPTION_MODEL_WHISPER` environment variable in the [update_config.sh](service/update_config.sh) script, which updates the `vigenair` Cloud Function's runtime variables. Likewise for Gemini, the `CONFIG_TRANSCRIPTION_MODEL_GEMINI` variable defaults to `gemini-1.5-flash` and can be set to any supported [Gemini model](https://cloud.google.com/vertex-ai/generative-ai/docs/learn/models#gemini-models) on Vertex AI. Finally, the transcription output is stored in an `input.vtt` file, along with a `language.txt` file containing the video's primary language, in the same folder as the input video.
